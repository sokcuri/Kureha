...
var App = {
				...
				if(tweet.text)
				{
					...
					if(App.config.home_timeline_noti)
					{
						...
						var noti = new Notification(tweet.user.name,
							{tag: seq, body: tweet.text, icon: tweet.user.profile_image_url_https});
	...
	getAboutme: function()
	{ 
		Client.get('https://api.twitter.com/i/activity/about_me.json', {cards_platform:'Web-12', include_cards:'1', model_version:'7', count: 600, since_id:App.getAboutme.max_position}, function(error, event, response){
			if (!error) {
				if(event.length)
				{
...
function addRow(element, msg) {
	...
	div.innerHTML += tweet_div.format(msg);
					 //<!--<input type="button" value="-" onclick="removeRow(this.parentNode)">//-->
	
	// Tweet Insert
	var sp2 = element.firstElementChild.firstElementChild;
	element.firstElementChild.insertBefore(div, sp2);

}

function addActivity(element, event) {
	// duplicate check
	var div = document.createElement('article');
	div.className = 'tweet';
...
function addTweet(element, tweet) {
	...
	if(element == home_timeline && tweet_count >= 200)
	removeTweets(home_timeline, 100);

	// duplicate check
	var item = document.querySelector('[data-tweet-id="' + tweet.id_str + '"]');
	if(item)
	{
		//console.log("duplicated tweets : " + tweet.id);
		return;
	}
	tweet_count++;
	info_box.innerHTML = "tweet: " + tweet_count;
/*
	client.get('conversation/show.json', {count: 2, id: tweet.id_str}, function(error, event, response){
	if (!error) {
		
		Array.from(event).forEach(function(item) {
			//console.log('tweet.id_str: ' + tweet.id_str + ', item.id_str: ' + item.id_str);
			if(tweet.id_str == item.id_str)
			{
				console.log("found" + tweet.id_str);
				//console.log(event);

				tweet.favorited = item.favorited;
				tweet.retweeted = item.retweeted;
				console.log("tweet.favorited:" + tweet.favorited + ", retweeted: " + tweet.retweeted);
			}
		});
	}
  else
...
*/
	var div = document.createElement('article');
	div.className = 'tweet';
	div.setAttribute('data-tweet-id', tweet.id_str);
	div.setAttribute('data-tweet-timestamp', tweet.timestamp_ms);
	//selectTweet 비활성화
	//div.setAttribute('onclick', 'selectTweet("https://twitter.com/' + tweet.user.screen_name + '/status/' + tweet.id_str + '")');
	...
	if(tweet.retweeted_status)
	{
		div.innerHTML += '<div class="retweeted_tweet">' + App.symbol.retweet + '<span class="retweeted_tweet_text">&nbsp;' + App.msg.someone_retweeted.format(tweet.user.screen_name, tweet.user.name) + '</span></div>\r\n';
		tweet = tweet.retweeted_status;
	}
	...
	var tweet_div = '<img class="profile-image" src={0}></img>\r\n' +
					'<div class="tweet-name"><a href="#" onclick="openExternal(\'https://twitter.com/{1}\')">\r\n' +
					'<strong>{2}</strong>\r\n' +
					'<span class="tweet-id">@{1}</span></a></div>\r\n' +
					'<p class="tweet-text">{3}</p>';

	div.innerHTML += tweet_div.format(tweet.user.profile_image_url_https, tweet.user.screen_name, tweet.user.name, text);
					 //<!--<input type="button" value="-" onclick="removeRow(this.parentNode)">//-->
	
	if(tweet.entities.media)
	{
		div.innerHTML += '<div class="tweet-image"><img src="' + tweet.entities.media[0].media_url_https + '" /></div>';
	}


	//div.getElementById('tweet-href').onclick = function() { require('nw.gui').Shell.openExternal( this.href ); };


	div.innerHTML += '<div class="tweet-date"><a href="#" onclick="openPopup(\'https://twitter.com/' + tweet.user.screen_name + '/status/' + tweet.id_str + '\')">' + new Date(Date.parse(tweet.created_at)).format("a/p hh:mm - yyyy년 MM월 dd일") + '</a> · ' + tagRemove(tweet.source) + '</div>';
	if (!tweet.retweet_count)
		tweet.retweet_count = "";
	if (!tweet.favorite_count)
		tweet.favorite_count = "";
	
	div.innerHTML += '<div aria-label="트윗 작업" role="group" class="tweet-task">' + 
					 '<div class="tweet-task-box"><button aria-label="답글" data-testid="reply" type="button">' + 
					 '<span>' + App.symbol.reply + '</span></button></div><div class="tweet-task-box ' + retweeted + '" data-retweet="' + id_str_org + '"><button aria-label="리트윗 1회. 리트윗" data-testid="retweet" type="button" onclick="retweet(\'' + id_str_org + '\')"><span class="tweet-task-count">' + App.symbol.retweet + '&nbsp;<span><span data-retweet-count="' + id_str_org + '">' + tweet.retweet_count + '</span></span></span></button></div><div class="tweet-task-box ' + favorited + '" data-favorite="' + id_str_org + '"><button aria-label="마음에 들어요" data-testid="like" type="button" onclick="favorite(\'' + id_str_org + '\')"><span>' + App.symbol.like + '&nbsp;<span class="tweet-task-count"><span data-favorite-count="' + id_str_org + '">' + tweet.favorite_count + '</span></span></span></button></div></div>';
	...
	if (!scrolling)
	{
		...
		if (element.scrollTop)
			element.scrollTop += element.firstElementChild.firstElementChild.getClientRects()[0].height + 10;

	}
}
function removeTweets(element, count) {
	var i = 0;
	...
	tweet_count = home_timeline.firstElementChild.childElementCount;
	info_box.innerHTML = "tweet: " + tweet_count;
}

// Remove to tweet
function removeTweet(element, input) {
	element.removeChild( input );
}
/*
function selectTweet(href)
{
...
function retweet(e)
{
	...
	if(!document.querySelector('[data-retweet="' + e + '"]').classList.contains('retweeted'))
	{
		document.querySelector('[data-retweet="' + e + '"]').classList.add('retweeted');
		document.querySelector('[data-retweet-count="' + e + '"]').innerText++;
		Client.post('statuses/retweet/' + e, function(error, tweet, response) {
			if (error) {
				document.querySelector('[data-retweet="' + e + '"]').classList.remove('retweeted');
				if(document.querySelector('[data-retweet-count="' + e + '"]').innerText == "1")
					document.querySelector('[data-retweet-count="' + e + '"]').innerText = "";
				else
					document.querySelector('[data-retweet-count="' + e + '"]').innerText--;
			}
...
function favorite(e)
{	
	if(!document.querySelector('[data-favorite="' + e + '"]').classList.contains('favorited'))
	{
		document.querySelector('[data-favorite="' + e + '"]').classList.add('favorited');
		document.querySelector('[data-favorite-count="' + e + '"]').innerText++;
		Client.post('favorites/create', {id: e}, function(error, tweet, response) {
			if (error) {
				document.querySelector('[data-favorite="' + e + '"]').classList.remove('favorited');
				if(document.querySelector('[data-favorite-count="' + e + '"]').innerText == "1")
					document.querySelector('[data-favorite-count="' + e + '"]').innerText = "";
				else
					document.querySelector('[data-favorite-count="' + e + '"]').innerText--;
			}
